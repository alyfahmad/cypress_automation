describe("Verify login page", { testIsolation: true }, () => {
  ...
  it("Validate login page content", () => {
    ...
    cy.xpath(
      "//h5[contains(normalize-space(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')), 'login')]"
    ).should("be.visible");
    cy.xpath("//div[@class='orangehrm-login-logo']").should("be.visible");
    cy.xpath(
      "//p[contains(normalize-space(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')), 'orangehrm os')]"
    ).should("be.visible");
    cy.xpath(
      "//p/a[contains(normalize-space(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')), 'orangehrm, inc')]"
    ).should("be.visible");
    //verify socials
  ...
  it("Validate user cannot login without credentials", () => {
    ...
    cy.xpath(
      "//input[@placeholder='Username']//parent::div//parent::div/span[contains(normalize-space(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')), 'required')]"
    ).should("be.visible");
    cy.xpath(
      "//input[@placeholder='Password']//parent::div//parent::div/span[contains(normalize-space(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')), 'required')]"
    ).should("be.visible");
  });
  ...
  it("Validate user cannot login with wrong credentials", () => {
    ...
    cy.xpath("//button[contains(normalize-space(.),'Login')]").click();
    cy.xpath(
      "//p[contains(normalize-space(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')), 'invalid credentials')]"
    ).should("be.visible");
  });
